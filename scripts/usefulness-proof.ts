...
import { buildDeterministicConsultantView, type ConsultantSelectionRecord } from '../src/consultantViews.js';
...
// ---- 4. source anchors for every selected record, straight off the applied register.
const anchorStatement = db.prepare('SELECT speaker, t_ms, quote, verified FROM register_row_anchors WHERE project_id = ? AND external_register_id = ? ORDER BY t_ms');
function anchorsFor(externalId: string) {
  return (anchorStatement.all(project.id, externalId) as Array<{ speaker: string | null; t_ms: number | null; quote: string | null; verified: number }>)
    .map((row) => ({ speaker: row.speaker, tMs: row.t_ms, quote: row.quote, verified: Boolean(row.verified) }));
}

const anchorsById = new Map<string, ReturnType<typeof anchorsFor>>();
for (const record of [...meeting.records, ...needsWarwick.records]) {
  if (!anchorsById.has(record.id)) anchorsById.set(record.id, anchorsFor(record.id));
}
writeFileSync(path.join(outDir, 'source-anchors.json'), JSON.stringify(Object.fromEntries(anchorsById), null, 2), 'utf8');
